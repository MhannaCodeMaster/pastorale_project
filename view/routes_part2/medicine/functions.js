...
function deleteAllPatient(patientID) {
    return new Promise((resolve, reject) => {
        let SQL = `DELETE FROM medicine_taken WHERE id_beneficiary = '${patientID}'`
...
function reInsertPatient(req) {
    return new Promise(async(resolve, reject) => {
        let SQL = "INSERT into medicine_taken set ?"
        ...
        if (req.body.sicknessqty > 1) {
            ...
            for (let i = 0; i < req.body.sicknessqty; i++) {
                ...
                let item = {
                    id_medicine_taken: newid,
                    id_beneficiary: req.body.patientid,
                    id_sickness: req.body.sicknesspost[i],
            ...
            let item = {
                id_medicine_taken: newid,
                id_beneficiary: req.body.patientid,
                id_sickness: req.body.sicknesspost,
            ...
            db.query(SQL, item, (err, result) => {
                if (!err) {
                    ...
                    let item2 = {
                        ...
                        id_beneficiary: req.body.patientid,
                        medReceiptID: result.insertId,
                        description: `${temp.sourceCodes.find(e=>e.sourceCode == req.body.sourcepost).sourceName}: ${temp.medications.find(e=>e.id_medicine == req.body.medicationpost).description_medicine}`,
                        transferDateAdded: d.getDate() + "-" + (d.getMonth() + 1) + "-" + d.getFullYear() + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds(),
                    }
