            ...
            for(let i=0; i<data.currencies.length; i++){
                ...
                if(currentTransfers.length > 0){
                    ...
                    for(let x=0; x<currentTransfers.length; x++){
                        ...
                        if(data.beneficiaries.find(e=> e.id_beneficiary == currentTransfers[x].id_beneficiary)){
                            var currentBeneficiary = data.beneficiaries.find(e=> e.id_beneficiary == currentTransfers[x].id_beneficiary)
                        }
                        if(data.donators.find(e=> e.id_donator == currentTransfers[x].id_donator)){
                            ...
                            if(currentBeneficiary){
                                ws.cell(x+2,5)
                                .string(currentBeneficiary.name_beneficiary + " " + currentBeneficiary.father_name_beneficiary + " " + currentBeneficiary.family_name_beneficiary)
                                .style(styleBody)
                            } else {
                                ws.cell(x+2,5)
                                .string("")
        ...
        if(req.body.inclusions.includes("medicationreceipt")){
            ...
            let sources = await getSourceCodes()
                ...
                for(let i=0; i<beneficiaries.length; i++){
                    let currentBeneficiary = beneficiaries[i].name_beneficiary
                    let currentReceipts = data.medReceipts.filter(e=>e.id_beneficiary == beneficiaries[i].id_beneficiary)
