...
for (var i = 0; i < currencyButton.length; i++) {
    ...
    currencyButton[i].addEventListener("click", (event) => {
        ...
        if(data.queries.month){
            window.location.replace(`/account?id_currency=${currentID}&r=1&month=${data.queries.month}`);
            return
        }
        ...
        window.location.replace(`/account?id_currency=${currentID}&r=1&month=${month}`);
    })
}

document.getElementById("add-currency").addEventListener("click", (event) => {
    window.location.replace(`/account/addCurrency?id_currency=${data.queries.id_currency}`);
})